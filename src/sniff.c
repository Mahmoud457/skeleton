...
#define ETH_HLEN 14 //Defines eth might have to change when on DCS

HashSet* synIps;
atomic_int synCount = 0;
atomic_int ArpCount = 0;
atomic_int bbcCount = 0;
atomic_int googleCount = 0;

static volatile int stopSniffing = 0;
...
void sniff(char *interface, int verbose) {
  ...
  HashSet* synIps = createHashSet(32, 0.75);
  ...
  while (!stopSniffing) {
    ...
    if (packet == NULL) {
      ...
      dispatch(&header, packet, verbose, synIps, &synCount, &ArpCount, &googleCount, &bbcCount);
